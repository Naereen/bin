...
#curl -X POST -s --data-urlencode "input=`cat ${script}`" http://javascript-minifier.com/raw > "${desti}" && \

curl -v -X POST -s --data-urlencode input@"${script}" http://javascript-minifier.com/raw > "${desti}" && \
